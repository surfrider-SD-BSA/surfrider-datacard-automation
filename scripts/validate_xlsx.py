...
def find_cell(xml, ref):
    m = re.search(rf'<c r="{re.escape(ref)}"(?=[\s/>])', xml)
    if not m:
...
CARDS = [
    ...
    {"cardNumber": 3, "pages": [5, 6], "type": "tally", "values": [(18, 7, 0.40), (80, 2, 0.35)]},
    # Last usable column (BZ) -- boundary check.
    {"cardNumber": 76, "pages": [151, 152], "type": "total", "values": [(18, 1, 0.99)]},
]
...
def main():
    ...
    check(
        sheet.count('t="shared"') == shared_before == 83,
        f"all {shared_before} shared formulas preserved",
    )
    check(
        '<f t="shared" ref="B18:B33" si="1">SUM(C18:BZ18)</f>' in sheet,
    ...
    for c in tree.iter(f"{{{ns['m']}}}c"):
        ...
        if v is not None and v.text is not None:
            ...
            if i is not None:
                cells[c.get("r")] = (
                    "is",
                    "".join(t.text or "" for t in i.iter(f"{{{ns['m']}}}t")),
                    None,
                )
    ...
    check(cells.get("A6") == ("is", "8.2.25", None), "event date -> A6 in the chapter's format")
    check(cells.get("A8") == ("is", "Ocean Beach", None), "shoreline -> A8")
    check(cells.get("A10") == ("v", "5", None), "volunteer count -> A10")
    check(
        cells.get("A4") == ("is", "San Diego CH54 <test@example.org>", None),
        "XML-escaped club field round-trips",
    )
    check(not any(f"B{r}" in edits for r in range(1, 17)), "no header value written into column B")
    ...
    clobbered = [
        f"B{r}" for r in sorted(item_rows) if re.search(rf'<c r="B{r}"[^>]*>(?!<f)', sheet)
    ]
    check(not clobbered, f"no column B formula overwritten ({len(item_rows)} rows checked)")

    check(
        'fullCalcOnLoad="1"' in out.read(WORKBOOK).decode(),
        "fullCalcOnLoad set so column B recalculates",
    )
    check(
        'name="Provenance"' in out.read(WORKBOOK).decode(),
        "provenance sheet registered in workbook",
    )
    check(
        "/xl/worksheets/sheet2.xml" in out.read(TYPES).decode(),
    ...
    missing = [
        t
        for t in re.findall(r'Target="([^"]+)"', out.read(RELS).decode())
