    ...
    for c in sheet.iter(f"{{{NS['m']}}}c"):
        ...
        if v is None or v.text is None:
            ...
            if inline is not None:
                cells[ref] = "".join(t.text or "" for t in inline.iter(f"{{{NS['m']}}}t"))
            continue
...
def main():
    ...
    for key, row, label in HEADER_ROWS:
        lines.append(
            f"  {key}: {{ labelRow: {row}, valueRow: {row + 1}, label: {ts_string(label)} }},"
        )
    lines.append("} as const;")
    lines.append("")
    ...
    lines.append("export const MAX_VOLUNTEERS = LAST_DATA_COLUMN - FIRST_DATA_COLUMN + 1;")
    lines.append("")
