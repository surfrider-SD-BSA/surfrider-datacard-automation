...
`ios/testflight.sh` does the whole build and upload.

**This is set up and in use.** The chapter has an Apple Developer Program
membership, an `Apple Distribution` certificate on team `P45J6R7ZHD`, and an app
record — *Data Cards*, Apple ID `6807599166`, bundle
`com.mateobesse.surfriderdatacards`. Builds archive, sign, upload and reach
testers; there is an external group with a public link, and volunteers have
installed from it. The paragraph that used to stand here said none of that was
possible, which was true when it was written in August and stopped being true on
5 September 2026.

**One piece is still missing: the App Store Connect API key.** Without it
`testflight.sh` stops at its own first line, because `ASC_KEY_ID` and
`ASC_ISSUER_ID` are required. Until somebody generates one, the way a build has
actually been getting to TestFlight is Xcode's Organizer — archive with
`xcodebuild`, then Distribute App → App Store Connect → Upload, which signs in
with the Xcode account rather than a key. **Choose App Store Connect, not
TestFlight Internal Only**: Xcode remembers the last method used, and a build
uploaded as Internal Only is stamped `Internal` and can never be given to an
external tester.

What you have to do once, with what is already done marked:

1. ~~**Apple Developer Program membership**, $99/year.~~ **Done.** TestFlight is
   not available on a free account.
2. ~~**Pick a bundle identifier you own** and register an App ID for it.~~
   **Done**, as `com.mateobesse.surfriderdatacards`. It is still a personal
   identifier rather than one the chapter controls, and moving it later means a
   new app record and a new install for every tester, not an update.
3. ~~**Create the app record** in App Store Connect with that bundle ID.~~
   **Done** — Apple ID `6807599166`.
4. **Generate an App Store Connect API key** with the App Manager role, and put
   ...
   commit it. Nothing in this repository should ever contain it. **This is the
   one still outstanding**, and it is what stands between here and
   `./ios/testflight.sh` doing the upload instead of a person clicking through
   Organizer.
...
- ~~**Development signing only.**~~ **No longer true.** Distribution signing
  works, and builds are on TestFlight: `1788893453` is live to an external group
  with a public link. The seven-day free-team route described above still exists
  and is still the quickest way onto a plugged-in phone, but it is a fallback
  now rather than the only road. The identifier is still personal rather than
  the chapter's — see item 2 above.
